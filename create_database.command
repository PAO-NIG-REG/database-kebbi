...
# de-compresses and loads the Kebbi test data.
#
...
for sqlfile in $current_dir/schema/*.sql $current_dir/config/*.sql   $current_dir/extension/*.sql
do
...
# Extract the test data from the 7z archive and load it into the database.
    ...
    $zip_exe e -y -o"$data_path" "$data_path/kebbi.7z" >> $BUILD_LOG 2>&1
